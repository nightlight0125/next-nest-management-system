...
export default function RoleManagementPage() {
  ...
  const fetchData = async (page: number, pageSize: number) => {
    try {
      setLoading(true);
      const result = await fetchWrapper(`/role?page=${page}&limit=${pageSize}`);
      setData(
        result.data.map((item: RoleDataType) => ({
          ...item,
          key: item.id,
          menus: item?.roleMenus.map((menu) => menu.name).join(", "),
        }))
      );
      setPagination({
        ...pagination,
  ...
  const handleDelete = async (id: string) => {
    try {
      await fetchWrapper(`/role/${id}`, { method: "DELETE" });
      messageApi.destroy();
  ...
  const columns: ColumnsType<RoleDataType> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Link href={`/system-settings/role-management/${record.id}/edit`}>
  ...
  return (
    <div className="p-6">
      <div className="mb-4 flex justify-between items-center">
        ...
        <Space>
          <Link href="/system-settings/role-management/add">
